...
# Gene random number range (0 to GENE_RANGE)
GENE_RANGE = 1000

# Size of the population
POPULATION_SIZE = 8
# Number of games to run to evaluate a gene's fitness
GAMES_PER_GENE = 20
# Percent chance that a crossover will mutate
...
class AIPlayer(Player):
    ...
    def getNextGeneration(self, genes):
        ...
        # if every single game was lost, just create a brand new population
        if totalFitness == 0:
            # initialize the two populations
                for i in range(0, POPULATION_SIZE):
                    newPop.append(self.generateGene(self.geneSize))
        else:
