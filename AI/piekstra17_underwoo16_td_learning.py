...
class AIPlayer(Player):
    ...
    def __init__(self, inputPlayerId):        
        ...
        # when not learning, limit utility update rate
        if not LEARNING:
            self.td_alpha = 0.30
        ...
        # set the unique identifier for the AI (for during learning) (based on the alpha change values)
        self.AIID = str(self.td_alpha_change_rate) + '_' + str(self.td_alpha_change_amount).replace('.', '')+ "_SSFQF"
        
        # file name constant
        self.utilityFile = "piekstra17_underwoo16_td_learning.utilities" 
        
        # initialize the dictionary that will map state tuples to
        # their utilities
        self.utilityDict = {}
        
        # If the utilities file exists, load the contents into
        ...
        # RTD2 - Ruthless Temporal Difference-Learner 2.0
    ...
    #
    # Parameters: 
    #   state - the state to consolidate into a compressed
    #        (less information) state
    #
    # Return:
    ...
    def consolidateState(self, state):
        ...
        # get a reference to the enemy's inventory
        ...
        #initialize the compressed state dictionary
        compressedState = {}
        
            
        # keep track of health of enemy queen if exists
        if enemyQueen:
            compressedState["e"] = enemyQueen.health
        # keep track of the player queen's health if exists
        if playerQueen:
        ...
        if enemyQueen is None or len(enemyInv.ants) == 1 or playerInv.foodCount >= 11:
            ...
            # keep track of the number of ants 
            # (for making unique dictionary keys)
            antCount = 0
            
            # for each of the player's non-queen ants
            for ant in playerInv.ants:
                # if the ant is a worker
                if ant.type == WORKER:                    
                    # increment the ant count for the dictionary key 
                    antCount += 1
    ...
    #
    # Returns:
    #   The reward value of the state (-1.0, -0.01, or 1.0)
    ##
    ...
    #
    # Returns:
    #    The utilities dictionary object from the file where it was stored
    ##
    ...
    # addStateUtility
    #
    # Description: Adds a state to the utilities dictionary if it is
    #    not already there. Otherwise, updates the utility of the 
    #    currentState using the TD-Learning algorithm.
    #
    # Parameters:
    #    self - The object pointer
    #    currentState - The state to add or update if already in dict
    #    nextState - Optional parameter (used when updating 
    #                currentState's utility)
    #
    # Returns:
    #    The utility of the currentState (0 if currenState was just added)
    ##
    def addStateUtility(self, currentState, nextState=None):             
        # convert the current state to a compressed-state dictionary
        curStateDict = self.consolidateState(currentState)
        # convert the compressed state to tuple form to be used as a dictionary key
        curStateTuple = self.flattenDictToTuple(curStateDict) 
        
        # if currentState is not in the dictionary, 
        # add it with default utility of 0
        if nextState is None:
            if curStateTuple not in self.utilityDict:
                # keep track of the number of new states for
                # learning statistics
            ...
            # add the state to the dict if not there
            ...
            # if the next state is already in the dict
            else:
                # update the current state's utility using the TD-Learning algorithm
        ...
        # return the utility of the currentState
    ...
    #
    # Return: The move with the highest utility or a random
    #    move (based on a % chance) to explore as well as exploit
    #
    def getBestMove(self, currentState):
        # holds the best move
        bestMove = None
        # holds the utility of the best move (default really small)
        ...
        # return the best move found (highest utility)
        return bestMove
    ...
    def getMove(self, currentState):
        ...
        # return the move determined by getBestMove
    ...
    def registerWin(self, hasWon):
        ...
        if LEARNING:
            ...
            # holds the previous learning rate
            prev_alpha = self.td_alpha
            # calculate the amount to decrease the 
            # learning rate using our genious algorithm
            decreaseBy = (1.0 - (1.0/(math.e**(prev_alpha**self.td_alpha_change_rate))))*self.td_alpha_change_amount
            # decrease the learning rate
            self.td_alpha -= decreaseBy 
            
            # print out learning and state statistics
